...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <Card className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr] p-6">
        <div className="space-y-4">
          <div className="text-sm uppercase tracking-[0.2em] text-slate-500">Good Morning</div>
          <div>
            <div className="text-3xl font-semibold">{faculty.name}</div>
            <div className="text-sm text-slate-500 mt-1">{faculty.title} · Department of {faculty.department} · ID {faculty.employeeId}</div>
          </div>
        </div>
        <div className="flex flex-col justify-between rounded-3xl bg-slate-50 p-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="text-sm text-slate-500">Weekly Completion</div>
              <div className="text-3xl font-semibold">{weeklyCompletion}%</div>
            </div>
            <div className="h-20 w-20 rounded-full border-4 border-slate-200 flex items-center justify-center text-xl font-bold text-slate-900">
              {weeklyCompletion}%
            </div>
          </div>
          <div className="text-sm text-slate-500">Tasks + Projects combined</div>
        </div>
      </Card>
          ...
          <div className="divide-y">
            {tasks.map(t => (
              <div key={t.id} className="py-3 flex items-center justify-between">
                <div>
                  <div className={`text-sm ${t.status === 'completed' ? 'line-through text-gray-500' : ''}`}>{t.title}</div>
                  <div className="text-xs text-gray-400">Due {t.dueDate}</div>
                </div>
                <div className="flex items-center gap-3">
                  <Badge color={t.status === 'completed' ? 'green' : t.status === 'overdue' ? 'red' : 'yellow'}>{t.status}</Badge>
